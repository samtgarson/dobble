...
export const FinishedGame: FunctionComponent<FinishedGameProps> = ({
  game,
  user,
  players
}) => {
  ...
  const winner = useMemo(
    () => game.winner_id && players[game.winner_id],
    [game, players]
  )
  ...
  }, [client, game, router, user])

  useEffect(() => {
    Fathom.trackGoal('OQHSDU2R', 0)
  }, [])

  useAsyncFetch(({ nextId }) => client.getGame(nextId), setNextGame, null, {
    nextId: game?.next_game_id
  } as { nextId?: string })
  ...
  return (
    <Wrapper>
      {game.league && (
        <Heading>
          <strong>{game.league.name}:</strong> Game {game.league.game_count}
        </Heading>
      )}
      <Title>
        {winner.id === user.id ? '🎉 You won! ' : `😞 ${winner.name} won `}
        {duration && (
          <Title as='span' size={4} style={{ fontWeight: 400 }} subtitle>
            (in {duration})
          </Title>
        )}
      </Title>
      {game.league && (
        <Title as='h2' size={6} className='is-clearfix my-3'>
          {sortByLeague
            ? `${game.league.name} leaderboard`
            : "This game's results"}
          <Button
            className='is-pulled-right'
            size='small'
            onClick={() => setSort(!sortByLeague)}
          >
            {sortByLeague
      ...
      <ul className='mb-5'>
        <AnimateSharedLayout>
          {game.league && sortByLeague
            ? game.league.members.map((m, i) => (
                <LeaguePlayer key={m.id} m={m} i={i} />
              ))
