...
export class Dealer {
  ...
  private deal (pile: Deck) {
    ...
    logger.debug(`pile length after ${pile.length}`)
    logger.debug(`hand size ${handSize}`)
    logger.debug(`hands length ${hands.length}`)
    return hands
  }
}
